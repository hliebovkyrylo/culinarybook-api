...
class NotificationController {
  public async getAllMyNotifications(request: Request, response: Response) {
    ...
    const notifications = notificationService.getNotificationsByUserId(user.id);
  ...
  public async getAllMyUnreadedNotifications(request: Request, response: Response) {
    const user = request.user as User;
    const unreadedNotifications = notificationService.getUnreadedNotifications(user.id);
